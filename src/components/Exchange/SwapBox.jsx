...
import { useTokenAllowanceData } from "domain/legacy/hooks";
...
export default function SwapBox(props) {
  ...
  const { tokensAllowanceData } = useTokenAllowanceData(chainId, {
  ...
  const triggerRatio = useMemo(() => {
    if (!triggerRatioValue || triggerRatioValue === "") {
      return 0n;
    }
    try {
      ...
      // If parsing fails, return 0n as a safe fallback
      return 0n;
    }
  ...
  useEffect(() => {
    ...
    const token = toTokens.find((token) => token?.address === toTokenAddress);
    if (!token) {
    ...
    const updateSwapAmounts = () => {
      ...
      if (anchorOnFromAmount) {
        ...
        if (toToken) {
          ...
          if (nextToValue !== toValue) { // Only update if value has changed
      ...
      if (fromToken) {
        ...
        if (nextFromValue !== fromValue) { // Only update if value has changed
  ...
  const ERROR_TOOLTIP_MSG = {
    ...
    [ErrorCode.TokenPoolExceededShorts]: (
      <Trans>
        {shortCollateralToken && <p>{shortCollateralToken.symbol} is required for collateral.</p>}
        <p>
      ...
      <div className="Exchange-swap-info-group">
        ...
        {(isLong || isShort) && (
          <div className="Exchange-swap-market-box App-box App-box-border">
            ...
            {isShort && toTokenInfo && toTokenInfo.hasMaxAvailableShort && (
              <div className="Exchange-info-row">
                <div className="Exchange-info-label">
                  <Trans>Available Liquidity</Trans>
                </div>
