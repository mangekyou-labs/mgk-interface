...
import { ARBITRUM, AVALANCHE, AVALANCHE_FUJI, ETH_MAINNET, HEDERA_TESTNET } from "config/chains";

export const chainlinkClient = (chainId) => {
  if (chainId === HEDERA_TESTNET) {
    return createClient(HEDERA_TESTNET, "chainLink");
  }
  return createClient(ETH_MAINNET, "chainLink");
};
