...
export function getUsd(
  ...
  const decimals = expandDecimals(1, info.decimals);
  if (decimals === 0n) {
    return 0n;
  }
...
export function getTokenInfo(
  ...
  return {
    ...
    isWrapped: token.isWrapped || false,
    isStable: token.isStable || false,
    isShortable: token.isShortable || false,
  };
}
...
export function convertToTokenAmount(value: bigint, token: Token) {
  return value;
}
