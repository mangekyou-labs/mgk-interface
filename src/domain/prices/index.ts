...
import { HEDERA_TESTNET } from "config/chains";
...
export async function getLimitChartPricesFromStats(
  ...
  const statsChainId = chainId === HEDERA_TESTNET ? 42161 : chainId;

  const url = `${GMX_STATS_API_URL}/candles/${symbol}?preferableChainId=${statsChainId}&period=${period}&limit=${limit}`;
...
export function getChainlinkChartPricesFromGraph(chainId: number, tokenSymbol: string, period: string): Promise<FromOldToNewArray<Bar>> {
  ...
  for (let i = 0; i < CHUNKS_TOTAL; i++) {
    ...
    requests.push(chainlinkClient(chainId).query({ query }));
  }
  ...
  return Promise.all(requests)
    .then((chunks) => {
      ...
      chunks.forEach((chunk) => {
        chunk.data.prices.forEach((item) => {
          if (uniqTs.has(item.timestamp)) {
            return;
          }

          uniqTs.add(item.timestamp);
          prices.push([Number(item.timestamp), Number(item.price) / 1e8]);
        });
      });
  ...
  let { data: prices, mutate: updatePrices } = useSWR(swrKey, {
    fetcher: async () => {
      try {
        ...
        try {
          return await getChainlinkChartPricesFromGraph(chainId, symbol, period);
        } catch (ex2) {
          // eslint-disable-next-line no-console
